...
std::shared_ptr<KvCommandReply> KvClerk::sendCommand(const std::string& operation, 
	...
	while (true) {
		...
		peers_[latest_leader_id_]->Call<KvCommandReply>(cmd, 
											std::bind([&reply, &cond](std::shared_ptr<KvCommandReply> response){
														reply = response;
														cond.notify();
														//printf("notify\n");
													}, std::placeholders::_1));
		reyao::MutexGuard lock(mutex);
...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < n; ++i) {
		...
		peers.push_back(std::make_shared<raftcpp::PolishedRpcClient>(peer_addr, &scheduler));
	}
